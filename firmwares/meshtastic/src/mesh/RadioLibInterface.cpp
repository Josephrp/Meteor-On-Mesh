...
#if defined(MESHTONIC_H4M_MCP_RADIO)
static bool meshtonicIsVirtualPin(uint32_t pin)
{
    return pin >= MESHTONIC_VPIN_CS && pin <= MESHTONIC_VPIN_BUSY;
}

void LockingArduinoHal::pinMode(uint32_t pin, uint32_t mode)
{
    if (meshtonicIsVirtualPin(pin)) {
        return;
    }
    ArduinoHal::pinMode(pin, mode);
}

void LockingArduinoHal::digitalWrite(uint32_t pin, uint32_t value)
{
    if (meshtonicIsVirtualPin(pin)) {
        meshtonicMcpDigitalWrite(pin, value);
        return;
    }
...
{
    if (meshtonicIsVirtualPin(pin)) {
        return meshtonicMcpDigitalRead(pin);
    }
