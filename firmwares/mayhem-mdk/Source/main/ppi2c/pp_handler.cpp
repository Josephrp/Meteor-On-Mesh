...
std::vector<uint8_t> PPHandler::on_send_ISR() {
    ...
    switch (command_state) {
        ...
        case (uint16_t)Command::COMMAND_GETFEAT_DATA_MESHTONIC: {
            // Extended payload for Meshtonic H4M
            meshtonic_board_t mb = {};
            extern PinConfig pinConfig; // from main
            if (pinConfig.getProfile() == PinConfig::BoardProfile::MESHTONIC_H4M) {
                mb.profile = 10;
                mb.radio_count = (uint8_t)pinConfig.getRadioCount();
                mb.sensor_mask = pinConfig.getSensorMask();
                mb.uart_mode = pinConfig.getUartMode();
            }
            return std::vector<uint8_t>((uint8_t*)&mb, (uint8_t*)&mb + sizeof(mb));
        }

        case (uint16_t)Command::COMMAND_RADIO_GET_STATUS: {
            extern SXRadioManager sxManager;
            radio_status_t rs = {};
            int act = sxManager.getActiveSlot();
            rs.slot = (uint8_t)((act >= 0) ? act : 0);
            rs.active = 1;
            auto st = sxManager.getSlotStatus(rs.slot);
            rs.present = st.present ? 1 : 0;
            rs.busy = st.busy ? 1 : 0;
            rs.dio1 = st.dio1 ? 1 : 0;
            rs.rst = st.rst_level ? 1 : 0;
            // richer fields left 0 for base; ep app can extend via packet events
            return std::vector<uint8_t>((uint8_t*)&rs, (uint8_t*)&rs + sizeof(rs));
        }

        case (uint16_t)Command::COMMAND_RADIO_SELECT: {
            // Expect first byte of incoming as desired slot (simple protocol)
            extern SXRadioManager sxManager;
            // The caller provides data in the request; here we just toggle to slot 0 as demo if no payload insight.
            // For real: parse the request payload.
            sxManager.selectSlot(0);
            radio_status_t rs = {};
            rs.slot = 0;
            rs.active = 1;
            auto st = sxManager.getSlotStatus(0);
            rs.present = st.present ? 1 : 0;
            rs.busy = st.busy ? 1 : 0;
            rs.dio1 = st.dio1 ? 1 : 0;
            rs.rst = st.rst_level ? 1 : 0;
