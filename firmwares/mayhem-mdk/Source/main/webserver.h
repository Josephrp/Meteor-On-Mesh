...
static esp_err_t post_req_handler_pinconfig(httpd_req_t* req) {
    ...
    if (changed) {
        ...
        // Detect Meshtonic H4M preset from pins and set typed profile + defaults
        bool isMeshtonic =
            (i2cSda == 5 && i2cScl == 6 && gpsRx == 7 &&
             i2cSdaSlave == 5 && i2cSclSlave == 6);
        if (isMeshtonic) {
...
// Accept decoded packet events from the vendored host LoRa Wideband Decoder (LWD) bridge.
...
static httpd_handle_t setup_websocket_server(void) {
    ...
    // Bridge endpoint for the vendored LoRa Wideband Decoder running on a host with HackRF.
    // POST normalized packet events here; they are fed to the LoraDecoder EPApp.
    httpd_uri_t uri_lwd_packet = {.uri = "/lwd/packet",
                                  .method = HTTP_POST,
                                  .handler = post_req_handler_meshtonic_packet,
                                  .user_ctx = NULL,
                                  .is_websocket = false,
                                  .handle_ws_control_frames = false,
                                  .supported_subprotocol = NULL};
